...
- `DEFAULT_EXTENSION_ID` in `packages/installer/src/shared/constants.ts` is empty by design (the ID varies: unpacked/dev = path-derived per machine, CWS build = fixed). If `--extension-id` is omitted, the installer auto-detects it via `extension-id-detector.ts` (scans Chromium profiles' `Preferences`/`Secure Preferences` for an extension whose manifest name contains "agenthub"). With `--yes` and exactly one match it's used automatically; otherwise the detected ID is *suggested* (re-run with `--extension-id <id>` to confirm) — it never silently registers an unconfirmed ID. With no match, the install fails safe with actionable guidance (binary installed, but registration is skipped — nothing corrupted). The throw lives in `host-registrar.ts` `generateManifest` and fires BEFORE any manifest/registry write.
