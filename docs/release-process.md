...
How to cut a new release of AgentHub.
...
|---|---|---|
| `agenthub-{platform}-{arch}` (or `.exe`) | `packages/native-host` | The bridge — long-lived process, owns the WS to the extension |
| `agenthub-helper-{platform}-{arch}` (or `.exe`) | `packages/native-host-helper` | Native-messaging endpoint Chrome calls for diagnostics, MCP registration check, and native-host spawn |
...
|---|---|---|
| Windows x64 | `agenthub-win-x64.exe` | `agenthub-helper-win-x64.exe` |
| macOS x64 | `agenthub-macos-x64` | `agenthub-helper-macos-x64` |
| macOS arm64 | `agenthub-macos-arm64` | `agenthub-helper-macos-arm64` |
| Linux x64 | `agenthub-linux-x64` | `agenthub-helper-linux-x64` |
| Linux arm64 | `agenthub-linux-arm64` | `agenthub-helper-linux-arm64` |
...
.\packages\native-host\bin\agenthub-win-x64.exe --version
.\packages\native-host-helper\bin\agenthub-helper-win-x64.exe --version
...
1. Verify the release page lists `agenthub-helper-*` assets:
   `gh release view vX.Y.Z`
...
- **`npm publish` for `agenthub-setup`** — the installer code on
  this branch is more recent than what's on npm (v0.1.2). If you want
  `npx agenthub-setup` to pull the latest installer, run
