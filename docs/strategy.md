...
about where Pilotwave competes and where it doesn't.

## TL;DR

1. **Pivot to chat-first.** The headline product is the in-extension chat tab, not the
   MCP server. Pilotwave can't beat browsermcp on MCP setup UX without becoming them.
2. **The chat tab doesn't compete with browsermcp.** browsermcp users still need a paid
   Claude Code/Cursor subscription to drive their browser. Pilotwave's chat is the
...
Pilotwave picked the opposite tradeoffs — native-host binary, native messaging registry +
...
This isn't fixable by polishing the installer. browsermcp avoided every Pilotwave setup
problem by **not having an installer at all**.

## Where Pilotwave still wins

These are real, not theoretical:

| Capability | browsermcp | Pilotwave |
|---|---|---|
...
chose a minimal tool set. Pilotwave exposed more.

## Connection lifecycle comparison

| Scenario | browsermcp | Pilotwave |
|---|---|---|
...
This is the one area where Pilotwave's complexity actually pays off. **browsermcp wins
first-time setup; Pilotwave wins day-2 reliability.** Most products live or die on
first-time setup, which is why browsermcp's UX feels better — but for power users who
restart their AI tool more than once a day, Pilotwave's self-healing is a real advantage.
...
- browsermcp users still need a Claude Code or Cursor subscription. Pilotwave's chat
...
- **Don't refactor away from native messaging right now.** Despite browsermcp's
  simpler transport, Pilotwave's existing transport actually wins on day-2 reliability.
...
A working installation went silent. `chrome://extensions` showed Pilotwave enabled,
