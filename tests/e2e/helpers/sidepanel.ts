 ...
 *   AGENTHUB_TEST_LLM_PROVIDER (openai|anthropic|gemini, default openai)
 *   AGENTHUB_TEST_LLM_KEY      (required to run the chat assertions)
 *   AGENTHUB_TEST_LLM_MODEL    (per-provider default below)
 */
...
export const readChatConfigFromEnv = (): ChatConfig | null => {
  const apiKey = process.env.AGENTHUB_TEST_LLM_KEY;
  if (!apiKey) return null;
  const provider = (process.env.AGENTHUB_TEST_LLM_PROVIDER ?? 'openai') as Provider;
  if (!['openai', 'anthropic', 'gemini'].includes(provider)) {
    throw new Error(`Bad AGENTHUB_TEST_LLM_PROVIDER="${provider}" — must be openai|anthropic|gemini`);
  }
  const model = process.env.AGENTHUB_TEST_LLM_MODEL ?? DEFAULT_MODEL[provider];
