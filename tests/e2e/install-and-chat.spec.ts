...
const collectProviderRuns = (): ProviderRun[] => {
  ...
  const openai = process.env.COPILOT_TEST_OPENAI_KEY;
  const anthropic = process.env.COPILOT_TEST_ANTHROPIC_KEY;
...
test.describe('install-and-chat', () => {
  ...
  test.beforeAll(() => {
    ...
    if (runs.length === 0) {
      test.skip(
        true,
        'No provider keys in env. Set COPILOT_TEST_OPENAI_KEY and/or COPILOT_TEST_ANTHROPIC_KEY to enable.',
      );
    }
  });
