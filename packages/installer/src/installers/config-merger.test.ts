...
describe('mergeConfig', () => {
  ...
  it('merges into existing config preserving all keys', () => {
    ...
    const result = mergeConfig(filePath, { mcpServers: { 'pilotwave': { command: 'test' } } });
    ...
    expect(written.mcpServers['pilotwave']).toEqual({ command: 'test' });
  });
...
describe('integration: full merge flow', () => {
  it('handles Claude Desktop config merge', () => {
    ...
    const result = mergeConfig(filePath, {
      mcpServers: {
        'pilotwave': { command: 'C:\\path\\to\\binary.exe', args: [] },
      },
    });
    ...
    const written = JSON.parse(readFileSync(filePath, 'utf-8'));
    expect(Object.keys(written.mcpServers)).toEqual(['filesystem', 'github', 'pilotwave']);
    expect(written.mcpServers.filesystem.command).toBe('npx');
    expect(written.mcpServers['pilotwave'].command).toBe('C:\\path\\to\\binary.exe');
  });
  ...
  it('merge then remove roundtrip', () => {
    ...
    // Merge
    mergeConfig(filePath, { mcpServers: { 'pilotwave': { command: '/binary' } } });
    let content = JSON.parse(readFileSync(filePath, 'utf-8'));
    expect(content.mcpServers['pilotwave']).toBeDefined();
    expect(content.mcpServers.existing).toBeDefined();

    // Remove
    const result = removeConfigEntry(filePath, 'pilotwave');
    expect(result.success).toBe(true);
    content = JSON.parse(readFileSync(filePath, 'utf-8'));
    expect(content.mcpServers['pilotwave']).toBeUndefined();
  ...
  it('handles VS Code settings.json merge with many existing settings', () => {
    ...
    const result = mergeConfig(filePath, {
      mcp: {
        servers: {
          'pilotwave': { command: '/path/to/binary', args: [] },
        },
      },
    });
    ...
    expect(written.mcp.servers['pilotwave'].command).toBe('/path/to/binary');
...
describe('removeConfigEntry', () => {
  ...
  it('removes entry from mcp.servers (VS Code format)', () => {
    ...
    const content = JSON.parse(readFileSync(filePath, 'utf-8'));
    expect(content.mcp.servers['pilotwave']).toBeUndefined();
  ...
  it('succeeds when file does not exist', () => {
    const result = removeConfigEntry(join(TEST_DIR, 'nonexistent.json'), 'pilotwave');
    expect(result.success).toBe(true);
  ...
  it('creates backup before modifying', () => {
    ...
    const original = JSON.stringify({ mcpServers: { 'pilotwave': {} } }, null, 2) + '\n';
  ...
  it('preserves indentation style', () => {
    ...
    removeConfigEntry(filePath, 'pilotwave');
  ...
  it('preserves trailing newline', () => {
    ...
    writeFileSync(filePath, JSON.stringify({ mcpServers: { 'pilotwave': {} } }) + '\n');
  ...
  it('removes from both formats if present in both', () => {
    ...
    writeFileSync(filePath, JSON.stringify({
      mcpServers: { 'pilotwave': { command: 'a' } },
      mcp: { servers: { 'pilotwave': { command: 'b' } } },
    }, null, 2) + '\n');
