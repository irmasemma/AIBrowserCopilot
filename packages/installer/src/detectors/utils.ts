...
/**
 * Check if a JSON config file has an existing `pilotwave` MCP entry.
...
export const hasExistingMcpEntry = (configPath: string): boolean => {
  ...
  try {
    ...
    if (content.mcpServers?.['pilotwave']) return true;

    // Check VS Code/Cursor format: mcp.servers
    if (content.mcp?.servers?.['pilotwave']) return true;
